...
## All that this file is doing is for a set of input token IDs, compute the per-token log-probabilities and return them.
...
if __name__ == '__main__':   
    ...
    def get_per_token_logps(input_ids):
        ...
        logits = ref_model(input_ids).logits  # (B, L, V) ## model perform a forward pass
        logits = logits[:, :-1, :]  # (B, L-1, V), exclude the last logit: it corresponds to the next token pred
        input_ids = input_ids[:, 1:]  # (B, L-1), exclude the first input ID since we don't have logits for it
        per_token_logps = []

        for logits_row, input_ids_row in zip(logits, input_ids): 
            log_probs = logits_row.log_softmax(dim=-1)  ## apply on the last dimension ie (V) vocabulary
    ...
    def do_upload():
        try:
            ...
            print("[REF_SERVER] Received upload request")
            ...
            print('receive', data['inputs'].shape, data['rewards'], data['gen_logps'].shape if 'gen_logps' in data else '')

            return b'tensor'
        except Exception as e:
            print("[REF_SERVER] Error in /upload:", e)
            return b'error'
    ...
    while True:
        ...
        data = [json.dumps(d['base']).encode(), tensor_to_bytes(d['inputs']), tensor_to_bytes(d['rewards']), tensor_to_bytes(per_token_logps)]
